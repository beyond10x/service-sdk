...
/// ESS-owned disposition, retained at the original SDK public path.
pub use ess_service_contract::synthesis::RequiredDisposition;
use ess_service_contract::synthesis::{
    CapabilityError, required_disposition as resolve_disposition,
};
use ess_service_contract::{
    command_fields as resolved_command_fields, view_fields as resolved_view_fields,
};
use ess_synth::{CapabilityKind, SynthesisPlan};
...
impl ServiceRuntimeIr {
    ...
    /// Exact digest of the closed SDK obligation catalog used for realization.
    pub fn obligation_catalog_digest(&self) -> &str {
        &self.0.obligation_catalog.digest
    }

    /// Provider-validated obligation implementations selected by the service definition.
    pub const fn obligations(&self) -> &BTreeMap<DefinitionId, ResolvedObligation> {
        &self.0.obligations
    }
...
struct RuntimeDocument {
    ...
    obligation_catalog: ObligationCatalogBinding,
    admission: Vec<AdmissionStage>,
    definition: ServiceDefinition,
    obligations: BTreeMap<DefinitionId, ResolvedObligation>,
...
/// Exact SDK obligation catalog bound into a generated runtime document.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ObligationCatalogBinding {
    /// SHA-256 of the complete ordered catalog contract.
    pub digest: String,
    /// Exact implementation versions selected by this service.
...
pub enum IntentStage {
    ...
    ConstructCommand,
    /// Apply ESS validation and every selected SDK obligation.
    Validate,
...
pub fn compile(
    ...
    let obligations = service_obligations::compile(definition).map_err(|errors| {
        RuntimeDiagnostics(
            errors
                .diagnostics()
                .iter()
    ...
    Ok(ServiceRuntimeIr(RuntimeDocument {
        ...
        obligation_catalog: ObligationCatalogBinding {
            digest: service_obligations::catalog_digest(),
            providers: obligations
                .values()
                .map(|obligation| obligation.provider.to_string())
                .collect(),
        },
...
fn required_disposition(
    ...
    match resolve_disposition(synthesis, kind, source) {
        Ok(disposition) => Some(disposition),
        Err(error) => {
            let code = match &error {
                CapabilityError::Cardinality { .. } => RuntimeCode::MissingSynthesisCapability,
                CapabilityError::Refused { .. } => RuntimeCode::RefusedSynthesisCapability,
            };
            diagnostics.push(RuntimeDiagnostic::new(code, path, error.to_string()));
            None
        }
    }
}
...
pub enum RuntimeCode {
    ...
    InvalidDefinition,
    /// An obligation does not resolve to complete SDK-provided behavior.
    InvalidObligation,
